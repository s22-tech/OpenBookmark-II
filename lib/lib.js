...
function bookmarkmove(subdir, bmlist, folderid) {
	if (bmlist == '') {
		alert('No Bookmarks selected.');
	}
	else {
		bookmark_move = window.open(subdir + '/bookmarks/move_bookmark.php?bmlist=' + bmlist + '&folderid=' + folderid, 'bookmarkmove', 'toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=500,height=450');
	}
}
...
function foldermove(subdir, folderid) {
	if (folderid == '' || folderid == '0') {
		...
		folder_move = window.open(subdir + '/folders/move_folder.php' /*?sourcefolder=' + folderid + '&t=' + Date.now() */, 'foldermove', 'toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=500,height=450');
	}
}
